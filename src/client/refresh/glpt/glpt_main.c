...
R_Register(void)
{
	...
	gl1_vertex_arrays = ri.Cvar_Get("gl1_vertex_arrays", "0", CVAR_ARCHIVE);
	...
	gl1_mtexcombine = ri.Cvar_Get("gl1_mtexcombine", "1", CVAR_ARCHIVE);
...
#define GET_PROC_ADDRESS(x) q##x = ( void * ) RI_GetProcAddress ( #x ); if (!q##x) { R_Printf(PRINT_ALL, #x " was not found!\n"); return -1; }
	...
	sscanf(gl_config.version_string, "%d.%d", &gl_config.version_major, &gl_config.version_minor);

	if (gl_config.version_major == 1)
	{
		if (gl_config.version_minor < 4)
		{
			QGL_Shutdown();
	...
	{
		R_Printf(PRINT_ALL, "...using GL_EXT_compiled_vertex_array\n");
		qglLockArraysEXT = ( void * ) RI_GetProcAddress ( "glLockArraysEXT" );
		qglUnlockArraysEXT = ( void * ) RI_GetProcAddress ( "glUnlockArraysEXT" );
	}
	else
	{
		R_Printf(PRINT_ALL, "...GL_EXT_compiled_vertex_array not found\n");
	}
...
RI_BeginFrame(float camera_separation)
{
	...
	if (gl1_overbrightbits->modified)
	{
		...
		{
			ri.Cvar_Set("gl1_overbrightbits", "2");
		}
