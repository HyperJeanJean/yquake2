...
R_UpdatePathtracerForCurrentFrame(void)
{
	...
	if (gl_pt_stats_enable->value)
	{
		end_ms = 0;//FIXME: Sys_Milliseconds();
...
R_PreparePathtracer(void)
{
	...
	R_Printf(PRINT_DEVELOPER, "R_PreparePathtracer: Static BSP texture size is %lux%lu\n", pt_bsp_texture_width, pt_bsp_texture_height);
...
PrintObjectInfoLog(GLhandleARB object)
{
	...
	if (info_log_length > 0)
	{
		...
		R_Printf(PRINT_ALL, "%s", info_log_buffer);
...
#define GET_PT_CVAR(x, d) x = ri.Cvar_Get( #x, d, CVAR_ARCHIVE); PT_ASSERT(x != NULL);
	GET_PT_CVAR(gl_pt_enable, "1")
	GET_PT_CVAR(gl_pt_stats_enable, "0")
	GET_PT_CVAR(gl_pt_bounces, "1")
