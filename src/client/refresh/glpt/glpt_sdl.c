...
int RI_InitContext(void* win)
{
	...
	sscanf(glver, "%d.%d", &gl_config.version_major, &gl_config.version_minor);

	if (gl_config.version_major < 1 || (gl_config.version_major == 1 && gl_config.version_minor < 4))
	{
		R_Printf(PRINT_ALL, "R_InitContext(): Got an OpenGL version %d.%d context - need (at least) 1.4!\n", gl_config.version_major, gl_config.version_minor);
